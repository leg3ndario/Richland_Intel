...
# Known Richland-area and common SC cities used as address anchors
_SC_CITIES = [
    "COLUMBIA", "BLYTHEWOOD", "IRMO", "ELGIN", "CHAPIN", "LEXINGTON",
    "WEST COLUMBIA", "CAYCE", "FOREST ACRES", "SPRINGDALE", "HOPKINS",
    "EASTOVER", "BALLENTINE", "PELION", "GASTON", "SWANSEA", "WINNSBORO",
    "CAMDEN", "SUMTER", "ORANGEBURG", "HARTSVILLE", "FLORENCE", "ROCK HILL",
    "FORT MILL", "LANCASTER", "NEWBERRY", "UNION", "GAFFNEY", "SPARTANBURG",
    "GREENVILLE", "ANDERSON", "BEAUFORT", "HILTON HEAD ISLAND", "HILTON HEAD",
    "MYRTLE BEACH", "CONWAY", "CHARLESTON", "NORTH CHARLESTON",
    "MOUNT PLEASANT", "MT PLEASANT", "SUMMERVILLE", "GOOSE CREEK",
    "HANAHAN", "LADSON", "AIKEN", "NORTH AUGUSTA", "GREER", "TAYLORS",
    "SIMPSONVILLE", "MAULDIN", "EASLEY", "CLEMSON", "SENECA", "WALHALLA",
]
_CITY_ALT     = "|".join(re.escape(c) for c in sorted(_SC_CITIES, key=len, reverse=True))
_CITY_ANCHOR  = re.compile(
    rf"({_CITY_ALT}),?\s+SC\s*,?\s*(\d{{5}}(?:-\d{{4}})?)",
    re.IGNORECASE,
)
...
def extract_property_address(text: str) -> tuple[str, str, str, str]:
    """
    Four-layer property address extractor covering all address formats
    found in Richland County / Columbia Star legal notices.

    Layer 1 – Explicit label ("Property Address:", "Physical Address:")
    Layer 2 – Alias phrases ("also known as", "commonly known as", "located at")
    Layer 3 – TMS/PIN-adjacent window (±200 chars around parcel number)
    Layer 4 – Full-text scan using known SC city list as anchor
    """
    # ── Layer 1: explicit label ──────────────────────────────────────────────
    for pat in [
        r"[Pp]roperty\s+[Aa]ddress\s*:?\s*",
        r"PROPERTY\s+ADDRESS\s*:?\s*",
        r"[Pp]hysical\s+[Aa]ddress\s*:?\s*",
    ]:
        m = re.search(pat, text, re.IGNORECASE)
        if m:
            ctx = text[m.end() : m.end() + 200]
            ctx = re.split(r"\bTMS\b|\bDerivation\b|\bTERMS\s+OF\b",
                           ctx, flags=re.IGNORECASE)[0]
            r = _addr_from_text(ctx)
            if r:
                return r

    # ── Layer 2: alias phrases ────────────────────────────────────────────────
    for pat in [
        r"(?:also|improved\s+property)\s+known\s+as\s+",
        r"commonly\s+known\s+as\s+",
        r"(?:located|situated)\s+at\s+",
    ]:
        m = re.search(pat, text, re.IGNORECASE)
        if m:
            r = _addr_from_text(text[m.end() : m.end() + 160])
            if r:
                return r

    # ── Layer 3: TMS/PIN-adjacent window ─────────────────────────────────────
    tms_m = re.search(
        r"(?:TMS|PIN|APN)[/#\s]*(?:No\.?\s*)?[:=]?\s*[RMrm]?\d[\dA-Z\-]+",
        text, re.IGNORECASE,
    )
    if tms_m:
        window = text[max(0, tms_m.start() - 200) : tms_m.end() + 200]
        r = _addr_from_text(window)
        if r:
            return r

    # ── Layer 4: full-text scan ───────────────────────────────────────────────
    r = _addr_from_text(text)
    if r:
        return r
...
# SCORER
# ══════════════════════════════════════════════════════════════════════════════
# Distress-signal scoring model (0–100):
...
def score_record(rec: dict) -> tuple[list[str], int]:
    ...
    legal = (rec.get("legal") or "").upper()
    # Full notice text may be stored in 'legal' or reconstructed from fields
    full  = " ".join([
        rec.get("owner", ""),
        rec.get("grantee", ""),
        rec.get("legal", ""),
    ...
    # ── +30  Tax delinquency ──────────────────────────────────────────────────
    TAX_TYPES = {"TAXDEED", "LNCORPTX", "LNIRS", "LNFED"}
    TAX_WORDS = ["TAX DEED", "DELINQUENT TAX", "TAX SALE", "TAX LIEN",
                 "IRS", "INTERNAL REVENUE", "DEPT OF REVENUE",
                 "CORP TAX", "FEDERAL TAX", "TAX COLLECTOR"]
    if dtype in TAX_TYPES or cat == "tax_lien" or any(w in full for w in TAX_WORDS):
        flags.append("Tax delinquency")
        score += 30

    # ── +25  Code violation ───────────────────────────────────────────────────
    # Columbia Star publishes code-violation / unsafe-structure orders;
    # also look for "ORDER" + property maintenance language in legal field
    CODE_WORDS = ["CODE VIOLATION", "PROPERTY MAINTENANCE", "UNSAFE STRUCTURE",
                  "UNSAFE BUILDING", "BUILDING CODE", "ZONING VIOLATION",
                  "ORDER TO VACATE", "CONDEMNED", "NUISANCE"]
    if any(w in full for w in CODE_WORDS):
        flags.append("Code violation")
        score += 25

    # ── +20  Probate filing ───────────────────────────────────────────────────
    PRO_TYPES = {"PRO"}
    PRO_WORDS = ["PROBATE", "ESTATE OF", "NOTICE TO CREDITORS",
                 "PERSONAL REPRESENTATIVE", "LETTERS TESTAMENTARY",
                 "INTESTATE", "DECEDENT", "DECEASED", "HEIR"]
    if dtype in PRO_TYPES or cat == "probate" or any(w in full for w in PRO_WORDS):
        flags.append("Probate filing")
        score += 20

    # ── +15  Multiple liens ───────────────────────────────────────────────────
    # Triggered when doc type is a lien AND there are signs of stacked debt
    # (multiple defendants, HOA + mortgage, mechanic + tax, etc.)
    LIEN_TYPES = {"LN", "LNMECH", "LNHOA", "MEDLN", "LNIRS", "LNCORPTX", "LNFED"}
    MULTI_LIEN_WORDS = ["MECHANIC", "HOA", "HOMEOWNER", "MEDICAID",
                        "INTERNAL REVENUE", "DEPT OF REVENUE",
                        "SECOND LIEN", "MULTIPLE LIEN", "JUNIOR LIEN"]
    lien_signals = (
        dtype in LIEN_TYPES
        or cat == "lien"
        or cat == "tax_lien"
        or sum(1 for w in MULTI_LIEN_WORDS if w in full) >= 2
    )
    # Also counts as "multiple" if this is a foreclosure with named lien-holders
    # (grantee list contains semicolons suggesting multiple defendants)
    grantee_raw = rec.get("grantee", "")
    multi_defendant = grantee_raw.count(";") >= 1 or grantee_raw.count("&") >= 1
    if lien_signals or multi_defendant:
        flags.append("Multiple liens")
        score += 15

    # ── +10  Divorce or bankruptcy ────────────────────────────────────────────
    DIV_BK_TYPES = {"DRJUD"}
    DIV_BK_WORDS = ["DIVORCE", "DISSOLUTION OF MARRIAGE", "FAMILY COURT",
                    "DOMESTIC", "SEPARATION", "BANKRUPTCY", "CHAPTER 7",
                    "CHAPTER 11", "CHAPTER 13", "TRUSTEE IN BANKRUPTCY",
                    "DEBTOR", "CREDITOR MEETING"]
    if dtype in DIV_BK_TYPES or any(w in full for w in DIV_BK_WORDS):
...
def _dedup_key(rec: dict) -> str:
    """
    Build a canonical deduplication key for a record.
    Priority:
      1. Case/doc number (most reliable — unique per legal filing)
      2. TMS/parcel number + doc_type (same property, same notice type)
      3. Normalised owner + prop_address + doc_type (name + address + type)
      4. Normalised owner + doc_type + filed date (no address available)

    All values are uppercased and stripped so minor formatting differences
    (spaces, punctuation) don't create false duplicates.
    """
    def norm(v: str) -> str:
        return re.sub(r"[\s\-.,#/]+", "", str(v or "")).upper()

    doc_num  = norm(rec.get("doc_num", ""))
    doc_type = norm(rec.get("doc_type", ""))
    owner    = norm(rec.get("owner", ""))
    prop     = norm(rec.get("prop_address", ""))
    filed    = norm(rec.get("filed", ""))

    # Key 1: case/doc number alone is globally unique
    if doc_num and len(doc_num) >= 6:
        return f"DOCNUM:{doc_num}"

    # Key 2: property address + doc type (same property, same notice type)
    if prop and len(prop) >= 6:
        return f"PROP:{prop}:{doc_type}"

    # Key 3: owner + doc type + filed date
    if owner and filed:
        return f"OWNER:{owner}:{doc_type}:{filed}"

    # Key 4: owner + doc type (weakest — only if nothing else available)
    if owner:
        return f"OWNER:{owner}:{doc_type}"

    # Fallback: keep the record (don't accidentally drop something with no key)
    return f"NOKEY:{id(rec)}"


def deduplicate(records: list[dict]) -> list[dict]:
    """
    Remove duplicate records using a tiered key strategy.
    When two records share the same key, keep the one with:
      - higher score, then
      - more fields populated (prop_address, mail_address, amount), then
      - the first one seen (earlier in the list = higher score after sort)
    Logs how many duplicates were dropped.
    """
    seen:   dict[str, dict] = {}
    dupes   = 0
    ...
    for rec in records:
        ...
        if key not in seen:
            ...
            dupes += 1
            existing = seen[key]
            # Replace if this record is richer
            def richness(r):
                return (
                    r.get("score", 0),
                    bool(r.get("prop_address")),
                    bool(r.get("mail_address")),
    ...
    log(f"Deduplication: {len(records)} → {len(result)} records ({dupes} duplicates removed)")
    return result
